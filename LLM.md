...
| Local incremental cache            | Skip work when inputs unchanged                                 | ◐ Partial. Content-addressed cache (`lib/cache`, sha256+zlib) wired into `lib/maven` (classpath) and `lib/aether` (manual-path link). Other SDKs still use mtime-only. Wiring proceeds SDK by SDK. |
...
| Affected-target detection          | `git diff` → reverse-dep walk → only-build-changed              | ✗ TODO. Roadmap entry.                                                                                  |
...
| Build telemetry                    | Per-module timing, cache hit rates, bottleneck analysis         | ✗ TODO. Roadmap.                                                                                         |
