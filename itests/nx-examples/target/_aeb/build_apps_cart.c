...
int build_apps_cart(void* s) {
void* b = build_begin(s, "apps/cart");
if ((intptr_t)b == 0) {
        {
            return 0;
        }
    }
build_load_deps_file(b, "../../react.deps.ae");
build_dep(b, "libs/cart/cart-page");
build_dep(b, "libs/shared/header");
build_dep(b, "libs/shared/styles");
    {
        void* _bcfg = map_new();
        _aether_ctx_push(_bcfg);
        {
ts_tsconfig(_aether_ctx_get(), "tsconfig.app.json");
ts_skip_lib_check(_aether_ctx_get());
        }
        _aether_ctx_pop();
        ts_tsc_project(b, _bcfg);
    }
}
